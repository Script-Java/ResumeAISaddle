...
async def refine_resume(
    ...
    if config.enable_keyword_injection:
        ...
        # Skip injection if already well-aligned (avoids unnecessary LLM call)
        if keyword_analysis.injectable_keywords and keyword_analysis.current_match_percentage < 70:
            logger.info(
                "Injecting %d keywords (match %.0f%%): %s",
                len(keyword_analysis.injectable_keywords),
                keyword_analysis.current_match_percentage,
            ...
            logger.info(
                "Skipping keyword injection (match %.0f%%, %d injectable)",
                keyword_analysis.current_match_percentage,
                len(keyword_analysis.injectable_keywords),
            )
