...
from app.prompts.templates import IMPROVE_SCHEMA_EXAMPLE, KEYWORD_EXTRACTION_SCHEMA
...
def _parse_changes_from_result(result: dict[str, Any]) -> list[ResumeChange]:
    """Parse changes from an LLM result dict, handling malformed entries."""
...
async def generate_resume_diffs(
    ...
    When *job_keywords* is ``None`` the function uses a combined prompt that
    extracts job keywords from the description AND generates diffs in a
    single LLM call (keyword extraction is folded in to save one round‑trip).

    Args:
        original_resume: Resume content (markdown)
        job_description: Target job description
        job_keywords: Extracted job keywords, or ``None`` to extract inline
    ...
    Returns:
        ImproveDiffResult with list of changes, strategy notes, and
        optionally extracted keywords (when job_keywords was None)
    """
    ...
    if job_keywords is not None:
        # Existing path: keywords already available, use diff-only prompt
        ...
        system_prompt = (
            ...
            "Output only valid JSON with targeted changes."
        )
        max_tokens_val = 3072
    else:
        # Combined path: extract keywords + generate diffs in one LLM call
        prompt = DIFF_AND_KEYWORDS_PROMPT.format(
            strategy_instruction=strategy_instruction,
            output_language=output_language,
            keyword_schema=KEYWORD_EXTRACTION_SCHEMA,
            job_description=sanitized_jd,
            original_resume=resume_input,
        )
        system_prompt = (
            "You are an expert resume editor and job analyst. "
            "Output only valid JSON with both keywords and changes."
        )
        max_tokens_val = 4096
    ...
    # Parse changes (shared logic)
    changes = _parse_changes_from_result(result)

    # Parse keywords from combined extraction when applicable
    ...
    if not result.get("changes") and "changes" not in result:
