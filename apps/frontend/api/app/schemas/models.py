...
class ImproveDiffResult(BaseModel):
    """LLM output: a list of targeted resume changes and optionally extracted keywords."""

    changes: list[ResumeChange] = Field(default_factory=list)
    strategy_notes: str = Field(default="")
    keywords: dict[str, Any] | None = Field(
        default=None,
        description="Job keywords extracted alongside diffs (populated when keyword extraction is bundled)",
    )
