...
def _get_pdf_renderer():
    """Lazy import of PDF renderer — playwright may not be installed (Vercel)."""
    global _pdf_renderer
    if _pdf_renderer is None:
        from app.pdf import PDFRenderError, render_resume_pdf
        _pdf_renderer = (render_resume_pdf, PDFRenderError)
...
async def _improve_preview_flow(
    *,
    ...
    # Determine whether cached keywords are still valid
    ...
    keywords_valid = bool(cached_keywords) and job_keywords_hash == content_hash
    original_resume_data = _get_original_resume_data(resume)
    response_warnings: list[str] = []

    # Step 1 — Generate diffs (& optionally extract keywords) in one LLM call
    if original_resume_data:
        diff_result = await generate_resume_diffs(
            original_resume=resume["content"],
            job_description=job["content"],
            # Pass None when keywords aren't cached so the LLM extracts them
            # alongside the diffs in a single call — saves one round-trip.
            job_keywords=cached_keywords if keywords_valid else None,
            language=language,
            prompt_id=prompt_id,
            original_resume_data=original_resume_data,
        )

        # When keywords were extracted inline, cache them for future requests
        if not keywords_valid and diff_result.keywords:
            job_keywords = diff_result.keywords
            try:
                db.update_job(
                    request.job_id,
                    {
            ...
            job_keywords = cached_keywords or {}
    ...
    # Multi-pass refinement (keyword injection disabled — handled inline by the
    # diff prompt; AI phrase removal and master alignment still run locally).
        ...
        if master_data:
            ...
            refinement_result = await refine_resume(
                ...
                config=RefinementConfig(
                    enable_keyword_injection=False,  # folded into diff prompt
                ),
            )
...
async def improve_resume_confirm_endpoint(
    ...
    try:
        ...
        if allowed_hashes and request_hash not in allowed_hashes:
            logger.warning("Resume confirm rejected due to preview hash mismatch.")
            raise HTTPException(
                status_code=400,
                detail="Invalid improved resume data. Please retry preview.",
            )
        elif not allowed_hashes:
            logger.warning(
                "Preview hash missing for job %s. Bypassing preview validation.",
                request.job_id,
            )
